...
def get_array_map(init_image):
    ...
    #下面进行边缘检测
    #边缘检测实验参数：低75高100；低100高200
    image_array = cv2.Canny(
      image_array,
      75, #canny_lower_threshold，于边缘检测的双阈值过程。低于这个值的梯度强度将被抑制，不会被认为是边缘
      100, #canny_higher_threshold，高于这个值的梯度强度将被认为是边缘
    )
...
#加斑点，有望改变背景几何结构
def add_random_spots(init_image, mask_image, num_spots=10, min_size=20, max_size=50, tint_color=(100, 0, 0), opacity=0.5,seed = None,face_repair_weight = 1.0 ):
    #设置种子
    np.random.seed(seed)
...
def background_replace(image, mask_image):
    new_image = cv2.imread("./picture/stand/black.png")
    image = cv2.cvtColor(np.array(image),cv2.COLOR_RGB2BGR)
...
def portrait_mask(in_image, mask_kind_list, in_mask=None):
    ...
    with vision.ImageSegmenter.create_from_options(options) as segmenter:
        ...
        #mask_image = black2white(mask_image)
